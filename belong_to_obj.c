...
int		belong_to_plane(t_general *gen, t_plane *pl, int *i)
{
	...
	t1 = dot_prv(pl->nm, pl->cd) - dot_prv(pl->nm, gen->scene.cdo);
	t2 = dot_prv(pl->nm, gen->scene.cdv) - dot_prv(pl->nm, gen->scene.cdo);
	t = t1 / t2;
...
int		belong_to_cyhead0(t_general gen, t_cylinder cy, t_vector *pcy, int fl)
{
	...
	t1 = dot_prv(cy.nm, h) - dot_prv(cy.nm, gen.scene.cdo);
	t2 = dot_prv(cy.nm, gen.scene.cdv) - dot_prv(cy.nm, gen.scene.cdo);
	t1 = t1 / t2;
	p = sum_vs(1, gen.scene.cdo, t1, sum_vs(1, gen.scene.cdv, -1, gen.scene.cdo));
