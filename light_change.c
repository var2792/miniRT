...
{ // на бодобии этой функции сделать видимость/наложение сфер
	...
	i = 0; // заходит в несуществ сферы и в текущюю???
	while (objects.sp[i].is)
	{
		if (i != num_sp)
		{
			...
			if (t[1] >= 0 && (k[0] != 0 && k[1] != 0))
			{
				...
				t[1] = (t[1] < 0.01 && t[1] > -0.01) ? -5 : (-k[1] - sqrt(t[1]))/ 2 / k[0]; //если D == 0 и t[0] < 0
				if (t[0] > 0 || t[1] > 0)
				{
					if ((t[0] < 0 || t[1] <= t[0]))
						temp = add_t_vecs(1, objects.l[num_l].coord, t[1], add_t_vecs( 1, p, -1, objects.l[num_l].coord));
					else if ((t[1] < 0 || t[0] < t[1]))
						temp = add_t_vecs(1, objects.l[num_l].coord, t[0], add_t_vecs( 1, p, -1, objects.l[num_l].coord));
					len = lenght_vecs(add_t_vecs(1, objects.l[num_l].coord, -1, temp));
					l1 = lenght_vecs(add_t_vecs(1, objects.l[num_l].coord, -1, p));
					if (len > 0 && len < l1)
						return (0.2);
				}
			}
...
float	light_change_sp(t_objscene objects, t_canvas scene, t_vector p, int i)
{
	...
	while (objects.l[num_l].is)
	{
		...
		if ((bright = scalar_product_vecs(norm, lig) / lenght_vecs(lig)) < 0)
			bright = 0;
		res_br += objects.l[num_l].brirat * bright * check_shadow(num_l, i, objects, p);
		//else
