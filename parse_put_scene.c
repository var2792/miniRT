...
unsigned int	ft_colorvec_unsint(t_vector *vec)
{
	unsigned int num;
	unsigned int r[2];
	unsigned int g[2];
...
void			ft_null_scene(t_objscene *scene)
{
	...
	scene->a.rat_amlig = 0.2;
	ft_write_xyz(&(scene->a.color), 255, 255, 255);
	scene->a.rgb_16 = ft_colorvec_unsint(&(scene->a.color));
	...
	ft_write_xyz(&(scene->l.coord), -40.0, 50.0, 0.0);
	scene->l.brirat = 0.6;
	ft_write_xyz(&(scene->l.color), 10, 0, 255);
	scene->l.rgb_16 = ft_colorvec_unsint(&(scene->l.color));
	//printf("it was l, and num = %u\n", scene->l.rgb_16);

	ft_write_xyz(&(scene->sp.coord), 0.0, 0.0, 20.6);
	scene->sp.diam = 12.6;
	ft_write_xyz(&(scene->sp.color), 10, 0, 255);
	scene->sp.rgb_16 = ft_colorvec_unsint(&(scene->sp.color));
	...
	scene->pl.rgb_16 = ft_colorvec_unsint(&(scene->pl.color));
	...
	scene->sq.rgb_16 = ft_colorvec_unsint(&(scene->sq.color));
	...
	scene->cy.diam = 14.2;
	scene->cy.heig = 21.42;
	scene->cy.rgb_16 = ft_colorvec_unsint(&(scene->cy.color));
	...
	scene->tr.rgb_16 = ft_colorvec_unsint(&(scene->tr.color));
