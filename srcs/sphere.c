...
int		see_sp_util(t_scene scene, t_vector ptr, float t[])
{
	...
	if (t[0] > EPS)
		...
		return (0);
	if (len_vec(sum_vs(1, p, -1, scene.cdo))
	- len_vec(sum_vs(1, ptr, -1, scene.cdo)) < -EPS)
		return (1);
	return (0);
}
...
int		check_see_sp(t_general gen, t_vector ptr, int num_sp)
{
	...
	while (temp)
	{
		if (i != num_sp)
		{
			sp = temp->content;
			p = sum_vs(1, gen.scene.cdv, -1, gen.scene.cdo);
			if ((t[1] = find_discr(p, sum_vs(1, gen.scene.cdo, -1, sp->cd),
			sp->d * sp->d / 4, &(t[0]))) > EPS)
...
float	check_shadow_sp(t_light num_l, int num_sp, t_scobjs objects, t_vector p)
{
	...
	while (temp)
	{
		...
		{
			sp = temp->content;
			if ((t[1] = find_discr(sum_vs(1, p, -1, num_l.cd), sum_vs(1,
			num_l.cd, -1, sp->cd), sp->d * sp->d / 4, &(t[0]))) > EPS)
				if ((t[0] > EPS && t[0] < 1 - EPS) ||
				(t[1] > EPS && t[1] < 1 - EPS))
					return (0);
		}
		temp = temp->next;
...
float	light_change_sp(t_general gen, t_vector p, t_sphere sp, int num_sp)
{
	...
	{
		num_l = temp->content;
		if ((bright = dot_prv(sum_vs(-1, p, 1, sp.cd),
		sum_vs(1, p, -1, num_l->cd)) / len_vec(sum_vs(1, p, -1, sp.cd))
		/ len_vec(sum_vs(1, p, -1, num_l->cd))) < 0)
			bright = 0;
		res_br += num_l->br * bright *
		check_shadow(*num_l, num_sp, gen.objs, p);
